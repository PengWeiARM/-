...
void  ShowErrMessage( uint16_t  ensure) 
{
  ...
  switch(ensure)
    ...
    case  0x00:
			...
			ILI9341_DispString_EN_CH(	0,60,"OK");
			Delay(0xFFFFFF);
		break;
		
	  case  0x01:
			
			ILI9341_DispString_EN_CH(	0,60,"数据包接收错误");
	  Delay(0xFFFFFF);
		break;
	  ...
	  case  0x04:
			...
			ILI9341_DispString_EN_CH(	0,60,"指纹图像太干、太淡而生不成特征");
	  Delay(0xFFFFFF);
		break;
	  
		case  0x05:
			
			ILI9341_DispString_EN_CH(	0,60,"指纹图像太湿、太糊而生不成特征");
		Delay(0xFFFFFF);
		break;
		
	  case  0x06:
			
			ILI9341_DispString_EN_CH(	0,60,"指纹图像太乱而生不成特征");
	  Delay(0xFFFFFF);
		break;
		
	  case  0x07:
			
			ILI9341_DispString_EN_CH(	0,60,"指纹图像正常，但特征点太少（或面积太小）而生不成特征");
	  Delay(0xFFFFFF);
		break;
		
	  case  0x08:
			
			ILI9341_DispString_EN_CH(	0,60,"指纹不匹配");
	  Delay(0xFFFFFF);
		break;
		
	  case  0x09:
			
			ILI9341_DispString_EN_CH(	0,60,"对比指纹失败，指纹库不存在此指纹！");
	  Delay(0xFFFFFF);
		break;
		
		case  0x0a:
			
			ILI9341_DispString_EN_CH(	0,60,"特征合并失败");
		Delay(0xFFFFFF);
		break;
		
		case  0x0b:
			
			ILI9341_DispString_EN_CH(	0,60,"访问指纹库时地址序号超出指纹库范围");
		Delay(0xFFFFFF);
		break;
		
		case  0x10:
			
			ILI9341_DispString_EN_CH(	0,60,"删除模板失败");
		Delay(0xFFFFFF);
		break;
		
		case  0x11:
			
			ILI9341_DispString_EN_CH(	0,60,"清空指纹库失败");
		Delay(0xFFFFFF);
		break;	
		
		case  0x15:
			
			ILI9341_DispString_EN_CH(	0,60,"缓冲区内没有有效原始图而生不成图像");
		Delay(0xFFFFFF);
		break;
		
		case  0x18:
			
			ILI9341_DispString_EN_CH(	0,60,"读写 FLASH 出错");
		Delay(0xFFFFFF);
		break;
		
		case  0x19:
			
			ILI9341_DispString_EN_CH(	0,60,"未定义错误");
		Delay(0xFFFFFF);
		break;
		
		case  0x1a:
			
			ILI9341_DispString_EN_CH(	0,60,"无效寄存器号");
		Delay(0xFFFFFF);
		break;
		
		case  0x1b:
			
			ILI9341_DispString_EN_CH(	0,60,"寄存器设定内容错误");
		Delay(0xFFFFFF);
		break;
		
		case  0x1c:
			
			ILI9341_DispString_EN_CH(	0,60,"记事本页码指定错误");
		Delay(0xFFFFFF);
		break;
		
		case  0x1f:
			
			ILI9341_DispString_EN_CH(	0,60,"指纹库满");
		Delay(0xFFFFFF);
		break;
		
		case  0x20:
			
			ILI9341_DispString_EN_CH(	0,60,"地址错误");
		Delay(0xFFFFFF);
		break;
		
		default :
			
			ILI9341_DispString_EN_CH(	0,60,"模块返回确认码有误");
		Delay(0xFFFFFF);
		break;	
  }
	Unlocking_Init(ret);
