...
uint8_t XPT2046_Get_TouchedPoint ( strType_XPT2046_Coordinate * pDisplayCoordinate, strType_XPT2046_TouchPara * pTouchPara )
{
	...
	strType_XPT2046_Coordinate strScreenCoordinate; ////液晶坐标结构体
...
void XPT2046_TouchDown(strType_XPT2046_Coordinate * touch)
{
	...
	switch (interface)
		{
			case 0:
				Touch_Button_Interface_Down(touch->pre_x,touch->pre_y);
			
			break;
			
			case 1:
				Touch_Button_Down(touch->pre_x,touch->pre_y);
			
			break;
			
			case 2:
				Touch_Button_Change_Down(touch->pre_x,touch->pre_y);
			
			break;
			
			case 3:
				Touch_Button_Finger_Down(touch->pre_x,touch->pre_y);
			
			break;
			
			case 4:
				Touch_Button_ID_Down(touch->pre_x,touch->pre_y);
			break;
			
			case 5:
				Touch_Button_ASR_Down(touch->pre_x,touch->pre_y);
			break;
...
void XPT2046_TouchUp(strType_XPT2046_Coordinate * touch) 
{
	...
	//Touch_Button_Up(touch->pre_x,touch->pre_y);
		...
		{
			case 0:
				Touch_Button_Interface_Up(touch->pre_x,touch->pre_y);
			...
			case 2:
				Touch_Button_Change_Up(touch->pre_x,touch->pre_y);
			
			break;
			
			case 3:
				Touch_Button_Finger_Up(touch->pre_x,touch->pre_y);
			
			break;
			
			case 4:
				Touch_Button_ID_Up(touch->pre_x,touch->pre_y);
			break;
			
			case 5:
				Touch_Button_ASR_Up(touch->pre_x,touch->pre_y);
			break;
...
{
	  static strType_XPT2046_Coordinate cinfo={-1,-1,-1,-1};//定义一个新的液晶坐标结构体
	
		if(XPT2046_TouchDetect() == TOUCH_PRESSED)      //触摸状态检测机返回值=1
		{
			//LED_RED;
			
			//获取触摸坐标
			XPT2046_Get_TouchedPoint(&cinfo,strXPT2046_TouchPara);//问题不在这儿，获取触摸坐标
			
			//输出调试信息到串口
			//XPT2046_DEBUG("x=%d,y=%d,pre_x=%d,pre_y=%d",cinfo.x,cinfo.y,cinfo.pre_x,cinfo.pre_y);
